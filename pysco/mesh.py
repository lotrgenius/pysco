    ...
    for i in prange(-1, ncells_1d - 1):
        ...
        for j in prange(-1, ncells_1d - 1):
            ...
            for k in prange(-1, ncells_1d - 1):
                kp1 = k + 1
                minus_aijk = -a[i, j, k]
                minus_bijk_2 = -b[i, j, k]
                result[i, j, k, 0] = invh * (
                    (minus_aijk + a[ip1, j, k] + f * (minus_bijk_2 + b[ip1, j, k]))
                )
                result[i, j, k, 1] = invh * (
                    (minus_aijk + a[i, jp1, k] + f * (minus_bijk_2 + b[i, jp1, k]))
                )
                result[i, j, k, 2] = invh * (
                    (minus_aijk + a[i, j, kp1] + f * (minus_bijk_2 + b[i, j, kp1]))
                )
    return result
