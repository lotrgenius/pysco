...
def linear(
    ...
    if param["compute_additional_field"] and "mog" == THEORY:
        raise ValueError(f"Linear should not be used for the MOG field")
...
def residual_error(
    ...
    #NEW FOR MOG
        ...
        return linmog.residual_error(x, b, q)
    else:
...
def restrict_residual(
    ...
    if (
        ...
        q = np.float32(param["mog_q"])
        if len(rhs) == 0:
            return mesh.minus_restriction(linmog.operator(x, b, q))
        else:
            return mesh.restriction(linmog.residual_with_rhs(x, b, q, rhs))
    else:
        if len(rhs) == 0:
...
def smoothing(
    ...
    #NEW FOR MOG
        ...
        if len(rhs) == 0:
            ...
            linmog.smoothing_with_rhs(x, b, q, n_smoothing, rhs)
    else:
        if len(rhs) == 0:
